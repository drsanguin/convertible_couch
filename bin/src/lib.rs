...
use convertible_couch_lib::{
    ...
    sound_settings::{SoundSettings, SwapDefaultOutputDeviceResponse},
};
use std::error::Error;
...
) -> Result<(SwapPrimaryDisplaysResponse, SwapDefaultOutputDeviceResponse), Box<dyn Error>> {
    configure_logger(args.log_level)?;

    let display_settings_response = display_settings
        .swap_primary_displays(&args.desktop_display_name, &args.couch_display_name)?;

    let sound_settings_response = sound_settings
